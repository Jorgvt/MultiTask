...
for epoch in tqdm(range(epochs), desc="Epoch"):
    ...
    for batch in tqdm(dst_train.iter(batch_size=batch_size, drop_last_batch=True), leave=False, desc="Batches"):
    ...
    avg_loss = jnp.mean(jnp.array(losses_b))
    avg_corr = jnp.mean(jnp.array(losses_b_c))
    avg_reg = jnp.mean(jnp.array(losses_b_r))
    avg_add = jnp.mean(jnp.array(losses_b_add))
    
    losses.append(avg_loss)
    losses_c.append(avg_corr)
    losses_r.append(avg_reg)
    losses_add.append(avg_add)
    ...
    wandb.log({
        ...
        "reg": avg_reg,
        "adding": avg_add,
        "sigma_c": jnp.exp(model.sigma_c.value),
        "sigma_r": jnp.exp(model.sigma_r.value),
    })
